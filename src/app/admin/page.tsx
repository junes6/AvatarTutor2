  ...
  return (
    <div className="px-5 py-6 pb-14">
      ...
      <div className="rounded-2xl bg-white/5 border border-white/10 p-4 mb-4">
        <div className="text-xs text-white/40 mb-2">연결된 제공자</div>
      ...
      <div className="rounded-2xl bg-amber-500/10 border border-amber-500/25 p-4 mb-4 text-center">
        ...
        <div className="text-[11px] text-white/45 mt-0.5">누적 원가 추정 (약 {Math.round(total * (data.pricing.usdToKrw ?? 1380)).toLocaleString()}원)</div>
      </div>

      {data.daily.length === 0 && <div className="text-center text-white/35 text-sm py-10">아직 기록된 사용량이 없어요</div>}

      <div className="space-y-2">
        {data.daily.map((d) => (
          <div key={d.date} className="rounded-2xl bg-white/5 border border-white/10 p-4">
            <div className="flex justify-between items-center mb-2">
              <span className="font-bold text-sm">{d.date}</span>
              <span className="text-sm font-black text-amber-300">
                ${d.costUsd.toFixed(3)} <span className="text-[10px] text-white/40">≈{d.costKrw.toLocaleString()}원</span>
              </span>
            </div>
            <div className="grid grid-cols-4 gap-1 text-center text-[10px] text-white/50">
              <div>
                <div className="text-white/80 font-semibold">{(d.llmInputTokens / 1000).toFixed(1)}K</div>
                입력 토큰
              </div>
              <div>
                <div className="text-white/80 font-semibold">{(d.llmOutputTokens / 1000).toFixed(1)}K</div>
                출력 토큰
              </div>
              <div>
                <div className="text-white/80 font-semibold">{(d.sttSeconds / 60).toFixed(1)}분</div>
                STT
              </div>
              <div>
                <div className="text-white/80 font-semibold">{(d.ttsChars / 1000).toFixed(1)}K자</div>
                TTS
              </div>
            </div>
          </div>
